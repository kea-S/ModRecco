...
def main():
    ...
    apiPathString = f"{NUSMODS}/{ACADEMIC_YEAR}/{MODULE_LIST_DB}"

    rawModuleListResponse = make_get_request(apiPathString)

    moduleListJson = json.loads(rawModuleListResponse)

    saveDataPathString = get_absolute_path("data/dev/raw/moduleList.json")

    ensure_path_exists(saveDataPathString)

    save_json(saveDataPathString, moduleListJson)
